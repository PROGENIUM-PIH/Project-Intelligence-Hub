...
export function ReportsClient({ markets, initiatives }: Props) {
  ...
  const allInitiatives = initiativeId === "ALL";
  ...
  function interpretPrompt() {
    ...
    const market = markets.find((m) => lower.includes(m.name.toLowerCase()) || hasToken(lower, m.code));
    const requestsAllInitiatives = /\ball\s+initiatives\b/i.test(lower) || /\bacross\s+(?:all\s+)?initiatives\b/i.test(lower) || /\binitiative\s+portfolio\b/i.test(lower);
    const explicitInitiativeMatch = lower.match(/\bid\s*0*(\d+)\b/i);
    let initiative = requestsAllInitiatives ? undefined : initiatives.find((i) => hasToken(lower, i.code) || lower.includes(i.name.toLowerCase()));
    if (!requestsAllInitiatives && !initiative && explicitInitiativeMatch) {
  ...
  return <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
    <Card><CardHeader><div className="flex items-center gap-2"><Sparkles className="h-5 w-5"/><CardTitle>Ask for a report</CardTitle></div><CardDescription>Describe the management question and prepare the report scope.</CardDescription></CardHeader><CardContent className="space-y-4">
      ...
      <div className="rounded-lg border bg-secondary/40 p-4 text-sm"><p className="font-medium">Report will cover</p><p className="mt-2"><span className="text-muted-foreground">Market:</span> {selectedMarket?.name ?? "—"}</p><p><span className="text-muted-foreground">Initiative:</span> {allInitiatives ? "All Initiatives" : selectedInitiative?.code ?? "—"}</p><p><span className="text-muted-foreground">Period:</span> Last {weeks} weeks</p></div>
