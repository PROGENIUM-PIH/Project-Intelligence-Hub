  ...
  if(market.code==="DE"){
    ...
    return <div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card><CardHeader><CardTitle className="text-base">Linked Initiatives</CardTitle></CardHeader><CardContent><div className="max-h-72 space-y-3 overflow-y-auto pr-2">{market.initiatives.map(mi=><Link key={mi.id} href={`/initiatives/${mi.initiativeId}`} className="block rounded-lg border p-3"><p className="text-sm font-medium">{mi.initiative.code} · {mi.initiative.name}</p><p className="text-xs text-muted-foreground">Local lead: {mi.localLead}</p></Link>)}</div></CardContent></Card>
        <Card><CardHeader><CardTitle className="text-base">Documents ({market.documents.length})</CardTitle></CardHeader><CardContent><div className="max-h-72 overflow-y-auto pr-2"><DocumentsList documents={market.documents}/></div></CardContent></Card>
      </div>
