  ...
  if(market.code==="DE"){
    ...
    return <div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card><CardHeader><CardTitle className="text-base">Linked Initiatives</CardTitle></CardHeader><CardContent><div className="h-40 space-y-3 overflow-y-scroll pr-2 [scrollbar-gutter:stable]">{market.initiatives.map(mi=><Link key={mi.id} href={`/initiatives/${mi.initiativeId}`} className="block rounded-lg border p-3"><p className="text-sm font-medium">{mi.initiative.code} · {mi.initiative.name}</p><p className="text-xs text-muted-foreground">Local lead: {mi.localLead}</p></Link>)}</div></CardContent></Card>
        <Card><CardHeader><CardTitle className="text-base">Documents ({market.documents.length})</CardTitle></CardHeader><CardContent><div className="h-40 overflow-y-scroll pr-2 [scrollbar-gutter:stable]"><DocumentsList documents={market.documents}/></div></CardContent></Card>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <Card><CardHeader><CardTitle className="text-base">Latest Meetings</CardTitle></CardHeader><CardContent className="space-y-3">{market.meetings.length===0&&<p className="text-sm text-muted-foreground">No meetings linked to Germany yet.</p>}{market.meetings.map(m=><div key={m.id} className="rounded-lg border p-3"><p className="text-sm font-medium">{m.title}</p><p className="text-xs text-muted-foreground">{format(m.date,"EEE, MMM d yyyy")} · {meetingTypeLabel(m.type)}</p>{m.notes&&<p className="mt-2 line-clamp-2 text-xs text-muted-foreground">{m.notes}</p>}</div>)}</CardContent></Card>
      </div>
