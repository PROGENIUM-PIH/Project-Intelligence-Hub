    ...
    <div className="overflow-x-auto pb-1">
      <div className="grid min-w-[760px] gap-1" style={{gridTemplateColumns:`190px repeat(${markets.length}, minmax(64px,1fr))`}}>
        {milestones.map((milestone,row)=>[
          ...
          ...markets.map(m=>{const milestoneIndex=milestoneProgressIndex[row];const current=Math.min(Math.max(m.milestoneCompleted,0),4);const complete=m.milestoneCompleted>=5||milestoneIndex<current;const active=m.milestoneCompleted<5&&milestoneIndex===current;const reached=complete||active;return <div key={`${m.id}-${milestone}`} title={`${m.name} · ${milestone} · ${complete?"Completed":active?"Current":"Upcoming"}`} className={`flex min-h-11 items-center justify-center rounded-md border transition-transform hover:scale-[1.04] ${complete?"border-[#0E3A2F] bg-[#0E3A2F]":active?"border-[#78FAAE] bg-[#78FAAE] ring-1 ring-[#78FAAE]/40":"border-[#D5DCDA] bg-[#E3E9E6]"}`}>{reached&&<Check className={`h-4 w-4 ${complete?"text-white":"text-[#0E3A2F]"}`} strokeWidth={3}/>}</div>})
        ])}
        <div />
        {markets.map(m=><div key={m.id} className="px-1 pt-2 text-center text-[10px] font-semibold leading-tight">{m.name}</div>)}
      </div>
    </div>
    <div className="mt-4 flex flex-wrap gap-4 border-t pt-3 text-[10px] text-muted-foreground"><span className="flex items-center gap-1.5"><i className="flex h-3 w-3 items-center justify-center rounded-sm bg-[#0E3A2F]"><Check className="h-2.5 w-2.5 text-white" strokeWidth={3}/></i>Completed</span><span className="flex items-center gap-1.5"><i className="flex h-3 w-3 items-center justify-center rounded-sm bg-[#78FAAE]"><Check className="h-2.5 w-2.5 text-[#0E3A2F]" strokeWidth={3}/></i>Current / reached</span><span className="flex items-center gap-1.5"><i className="h-2.5 w-2.5 rounded-sm border bg-[#E3E9E6]"/>Upcoming</span></div>
  </section>;
}
