...
export default async function ManagementReportPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return (
    <div className="report-shell -m-6 min-h-screen bg-[#F4F5F3] text-[#0E3A2F]">
      <style>{`
        .report-page{font-family:Arial,Helvetica,sans-serif}.report-card{break-inside:avoid}.report-table{width:100%;border-collapse:collapse}.report-table th{background:#0E3A2F;color:white;text-align:left}.report-table th,.report-table td{padding:9px 10px;border-bottom:1px solid #d9dfdc;font-size:11px;vertical-align:top}.report-kicker{letter-spacing:.16em;text-transform:uppercase;font-size:10px;font-weight:700}.report-source{font-size:10px;color:#5e716a}
      ...
      <main className="report-page mx-auto max-w-[1100px] space-y-5 px-8 pb-12">
        <section className="report-card overflow-hidden rounded-2xl bg-[#0E3A2F] text-white shadow-sm">
          ...
          <div className="p-8">
            ...
            <div className="mt-5 flex items-end justify-between gap-6">
              <div><h1 className="text-3xl font-semibold tracking-tight">{market?.name ?? "All Markets"} · {initiative?.code ?? "Portfolio"}</h1><p className="mt-2 max-w-2xl text-sm text-white/70">{initiative?.name ?? "Cross-initiative management overview"} · Last {weeks} weeks</p></div>
              <div className="text-right text-xs text-white/65">Generated<br/><span className="text-sm font-semibold text-white">{date(new Date())}</span></div>
            </div>
          </div>
        </section>

        <section className="grid grid-cols-4 gap-3">
          {[ ["Status", initiative ? pretty(initiative.status) : market ? pretty(market.status) : "-"], ["Progress", initiative ? `${initiative.progress}%` : "-"], ["Open Risks", String(openRisks.length)], ["Next Deadline", date(nextDeadline)] ].map(([k,v]) => <div key={k} className="report-card rounded-xl border border-[#dce2df] bg-white p-4"><div className="report-kicker text-[#6c7e77]">{k}</div><div className="mt-2 text-xl font-semibold">{v}</div></div>)}
        </section>

        <section className="report-card rounded-xl bg-white p-6 shadow-sm"><div className="report-kicker text-[#00A36C]">Executive Summary</div><p className="mt-3 text-sm leading-6 text-[#314b42]">{sourceSummary}</p></section>

        <section className="grid grid-cols-[1.15fr_.85fr] gap-5">
          <div className="report-card rounded-xl bg-white p-6 shadow-sm"><div className="report-kicker text-[#00A36C]">What happened</div><div className="mt-4 space-y-3">{(meetings.length ? meetings.slice(0,4).map((m) => ({ date:m.date, title:m.title, text:m.notes })) : scopedActivities.map((a)=>({date:a.createdAt,title:a.type,text:a.description}))).map((item,i)=><div key={i} className="border-l-2 border-[#78FAAE] pl-4"><div className="text-xs font-semibold">{date(item.date)} · {item.title}</div><p className="mt-1 line-clamp-3 text-xs leading-5 text-[#53675f]">{item.text}</p></div>)}</div></div>
          <div className="report-card rounded-xl bg-[#E8FFF1] p-6"><div className="report-kicker text-[#007A51]">Management Attention Required</div><div className="mt-4 space-y-4">{openRisks.length ? openRisks.slice(0,3).map((r,i)=><div key={r.id}><div className="text-sm font-semibold">{i+1}. {r.title}</div><p className="mt-1 text-xs leading-5 text-[#405b51]">{pretty(r.severity)} · Owner: {r.owner}. {r.description}</p></div>) : <p className="text-sm">No open structured risks are currently recorded.</p>}</div></div>
        </section>
        ...
        <section className="report-card rounded-xl bg-white p-6 shadow-sm"><div className="report-kicker text-[#00A36C]">Source Trail</div><div className="mt-4 grid grid-cols-2 gap-3">{meetings.slice(0,4).map(m=><div key={m.id} className="rounded-lg border border-[#dce2df] p-3"><div className="text-xs font-semibold">{m.title}</div><div className="report-source mt-1">Meeting Notes · {date(m.date)}</div></div>)}{scopedActivities.slice(0,4).map(a=><div key={a.id} className="rounded-lg border border-[#dce2df] p-3"><div className="text-xs font-semibold">{a.type}</div><div className="report-source mt-1">{a.actor} · {date(a.createdAt)}</div></div>)}</div></section>
